/*
 * Copyright (c) 2016. Marek Berkan
 */
package pl.mberkan.oca.part2;

/**
 * - Be able to write code that uses Java operators
 * - Be able to recognize which operators are associated with which data types
 * - Understand Java operator precedence
 * - Be able to write code that use parentheses to override operator precedence
 * - Understand if and switch decision control statement
 * - Understand loop statement
 * - Understand how break and continue can change flow control
 */
public class SExamEssentials {
}
